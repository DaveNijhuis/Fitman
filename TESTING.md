...
| `test_formulas.py` | Body composition formula calculations — pure unit tests, no HTTP; the whole-body values follow WLA25's derivation chain for three made-up people (fat mass rounded to one decimal first; water, muscle, bone, protein, skeletal muscle and SMI, subcutaneous fat, whole-kcal BMR), body age follows the per-sex body-fat bands including the vendor's female 45–46 % quirk and the under-10 rule, and WHR is no longer estimated (#344); without trunk impedance every derived field except skeletal muscle and SMI is still computed; visceral fat, trunk fat and trunk muscle follow WLA25 (trunk terms off), worked by hand for a made-up person, with visceral within 1–20 (#325); arm and leg fat and muscle follow WLA25's per-limb regressions, each limb moved only by its own readings, arm muscle no longer a share of lean mass, the left/right reconciliation for arms (0.3 kg) and legs (0.5 kg) whichever side is off, and the fat and muscle floors (#332). Matching Fitdays was checked outside this repo, since it needs real weigh-ins |
| `test_backfill_derived.py` | Stored measurements with raw inputs but no derived fields get them at startup, using the age at the time of the measurement; rows that already have derived values, or lack body fat, are left alone; a second run changes nothing; it runs in the app's startup (#325); rows derived before #344 get only muscle mass, bone mass and skeletal muscle, with no stored value overwritten, and a derived row without impedances is left as it is (#344) |
...
| `test_sessions.py` | Full workout session flow: start → log sets → end → list → get logs; edge cases (unknown session, already ended, 404); N+1 query regression guard (asserts ≤2 SELECT statements on list endpoint); deleting a session, active (discard) or finished (#336): 204, its logs and history entry gone, a record it set no longer listed, another user's session 404; a finished workout stays finished: a set logged to it is refused with 409 and not stored, and `GET /api/sessions/{id}` reports whether it has ended, 404 for a missing or another user's session (#338) |
...
| `test_prod_db_credentials.py` | The prod compose has no literal database password and, run through Docker Compose from a temporary copy with its own `.env`, refuses to start without `POSTGRES_PASSWORD` (pointing at the README), builds the backend's `DATABASE_URL` from the same password, and publishes Postgres on `127.0.0.1:5433` only; dev and E2E keep their fixed password; CI's prod build supplies one; `.env.example` asks for a generated value with no default; the README generates it, explains changing it on an existing database, and explains querying it (#335) |
...
| `test_migration_muscle_bone_mass.py` | The migration adding `muscle_mass_kg` and `bone_mass_kg` is the single head after `j6k8l0m2n4o5`; its own `downgrade()` then `upgrade()` run against the real schema inside a rolled-back transaction, leaving both columns nullable (#344) |
| `test_migration_scale_user_id.py` | The migration follows `i5j7k9l1m3n4` on the main line; its own `downgrade()` then `upgrade()` run against the real schema inside a rolled-back transaction, leaving a nullable `scale_user_id` with a unique constraint. `create_all` alone would pass every other test with no migration at all |
| `test_config_tests_need_no_database.py` | Runs the config-only modules in a real `pytest` subprocess pointed at a port nothing listens on: they pass and collection opens no connection; a database-backed module still passes against the real test database, so making the connection lazy didn't skip it (#283). A new config-only module is added to its list |
...
| `test_compose_launch.py` | Plain `docker compose config`, with no `-f`, resolves to the production stack (nginx build on port 80, database on 5433); no `compose.yaml` can take precedence; `docker-compose.prod.yml` is gone; production pins no project name, so existing volumes keep theirs; the dev file runs Vite under its own project `fitman-dev`; README, ARCHITECTURE and CI never run the old file; the README deploys with `docker compose up -d`, explains the switch for existing deployments and documents the dev stack; backups use the compose service rather than a container name (#339) |
| `test_update_script.py` | Runs `update.sh` against a real git deployment cloned from a local origin, with a fake `docker` on a bare PATH: an update lists what's coming, backs up with `pg_dump --clean --if-exists` before merging and restarting, and shows the migrations that ran; nothing new changes nothing unless `--rebuild`; a stopped stack updates without a backup; local changes to tracked files, a diverged clone, a setting the new version requires but `.env` lacks (checked before merging, so a re-run then works), and an empty backup each stop it with the code and database untouched; new optional settings are listed; a failed start prints how to go back to the previous commit and restore the backup; executable and ShellCheck-clean; the README points to it (#350) |
| `test_setup_script.py` | Runs `setup.sh` in a temporary copy of the repo with fake `docker`, `ss`, `sudo` and `tailscale` on a bare PATH: a fresh install gets hex secrets, keeps the rest of `.env.example`, starts with `docker compose up -d --build --wait` and prints where to create the account; an existing `.env` keeps its secrets and settings, the placeholder secret key is replaced, and a missing database password with an existing database stops before anything starts; taken ports are reported, naming the program or the Docker container that holds them, and moved; explicit and localhost-only ports; a running stack keeps its ports; the scale choice; tailscale serve only when asked, on 8443 when 443 is taken; keyboard answers, re-asked when invalid, and Enter for defaults; missing Docker or daemon access stops before `.env` is written; a failed start points at the logs; the script is executable and ShellCheck-clean (shellcheck-py in `requirements-dev.txt`); the README leads the install with it (#349) |
| `test_compose_ports.py` | Through Docker Compose on a copy of the production file: the web app defaults to port 80 on every interface and the database to `127.0.0.1:5433`; `FITMAN_HTTP_PORT` moves the web app, to a port or to `127.0.0.1:<port>`; `FITMAN_DB_PORT` moves the database, which stays on `127.0.0.1`, and an address there is refused; `.env.example` documents both; the README lists every stack's ports, shows a working Caddy `reverse_proxy` setup and tailscale serve on another HTTPS port (#347) |
| `test_compose_config.py` | Static parse of `docker-compose.dev.yml`: the frontend sets `VITE_API_PROXY_TARGET`, it names the `backend` service rather than `localhost`, and the host it names is a service the compose file actually defines |
| `test_db_volume_guard.py` | Asserts the production compose file defines the `db-guard` service, that it mounts the data volume and names `fitman.db` in its message, that postgres waits on its successful completion, and that the volume is **not** renamed — a rename would start existing instances on an empty database |
...
The schema is built by the session-scoped `database` fixture, which `client` requests, not at import (#283). Collection opens no connection, so the config-only modules (compose files, CI, lint config, the scale protocol) run with no database at all; `test_config_tests_need_no_database.py` keeps it that way. The `_test` guard runs inside that fixture, before anything is dropped.
...
| `src/__tests__/chartFormatters.test.ts` | `numericTooltipFormatter` formats numbers and numeric strings, and falls back rather than rendering `NaN` for a non-numeric value, an array or `undefined`; `ProgressPage` has no `as any` casts or `no-explicit-any` suppressions left (#272) |
...
| `src/pages/__tests__/ActiveWorkoutPage.test.tsx` | A stale in-progress marker (#338): resuming a workout that has since finished goes to its History entry, and one that no longer exists goes home, clearing the marker so Resume is no longer offered; a marker for a different workout is left alone, and so is this one when the check merely fails (offline); a set refused because the workout ended elsewhere goes to History; finishing clears the marker before the server ends the session, restores it with an error if that fails, and goes to History if it had already ended |
| `src/pages/__tests__/ProgressPage.test.tsx` | The body metrics list shows muscle mass and bone mass, and no longer offers the WHR estimate, even for an old stored value (#344) |
| `src/pages/__tests__/WorkoutDetailPage.test.tsx` | Deleting a finished workout: a confirmation names the workout and date and says it can't be undone; Cancel keeps it; Delete calls the API and returns to History; a failed delete shows the error and stays; opened by URL without the summary, it still works ("this workout") (#336) |
...
| `tests/workout.spec.ts` | Workout page loads after starting a session; finishing a session returns to home; a finished workout deleted from its detail page is gone from History (#336) |
