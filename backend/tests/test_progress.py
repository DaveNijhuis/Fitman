...
def test_balance_returns_muscle_groups(client: TestClient):
    ...
    session = client.post("/api/sessions", json={"session": "Push A"}, headers=headers).json()
    client.post("/api/logs", json={"session_id": session["id"], "exercise_id": ex["id"], "weight": 30.0, "reps": 10}, headers=headers)
...
def test_balance_skips_exercise_with_null_muscles(client: TestClient):
    ...
    db = SessionLocal()
    null_ex = Exercise(name="Test NULL muscles", muscles=None, session="Push A", position=99, type="weight", equip="Dumbbell")
    db.add(null_ex)
    db.commit()
    db.refresh(null_ex)

    session_obj = WorkoutSession(session="Push A", started_at=datetime.now(timezone.utc), ended_at=datetime.now(timezone.utc))
    db.add(session_obj)
    db.commit()
    db.refresh(session_obj)
    db.add(Log(session_id=session_obj.id, exercise_id=null_ex.id, weight=20.0, reps=8, logged_at=datetime.now(timezone.utc)))
    db.commit()
    db.close()
...
def test_balance_skips_exercise_with_malformed_muscles(client: TestClient):
    ...
    db = SessionLocal()
    bad_ex = Exercise(name="Test malformed muscles", muscles=",,", session="Push A", position=98, type="weight", equip="Dumbbell")
    db.add(bad_ex)
    db.commit()
    db.refresh(bad_ex)
    ...
    db.commit()
    db.refresh(session_obj)
    db.add(Log(session_id=session_obj.id, exercise_id=bad_ex.id, weight=20.0, reps=8, logged_at=datetime.now(timezone.utc)))
    db.commit()
    db.close()
